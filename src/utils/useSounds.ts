...
export default function useSounds(): (sound: SoundType) => void {
    ...
    useEffect(() => {
        ...
        const loadSounds = async () => {
            await popSoundObject.loadAsync(require("@assets/sounds/pop_2.wav"));
            popSoundRef.current = popSoundObject;
            await pop2SoundObject.loadAsync(require("@assets/sounds/pop_1.wav"));
            pop2SoundRef.current = pop2SoundObject;
            await winSoundObject.loadAsync(require("@assets/sounds/win.mp3"));
            winSoundRef.current = winSoundObject;
            await lossSoundObject.loadAsync(require("@assets/sounds/loss.mp3"));
            lossSoundRef.current = lossSoundObject;
            await drawSoundObject.loadAsync(require("@assets/sounds/draw.mp3"));
